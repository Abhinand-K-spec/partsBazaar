...
import {
    products, adminSalesData, adminTopParts, adminLowStock, adminUsers, brands
} from '../data/mockData';
...
function AddProductModal({ onClose }) {
    const [form, setForm] = useState({ name: '', brand: '', partType: '', price: '', stock: '', isRare: false });
            ...
            <div className="relative w-full max-w-lg bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                ...
                <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1.5">Part Type</label>
                            <select value={form.partType} onChange={e => setForm(f => ({ ...f, partType: e.target.value }))} className="w-full input-field">
                        ...
                        <div className="col-span-2">
                            <label className="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1.5">Compatible Models</label>
                            <input placeholder="iPhone 11, iPhone 11 (A2221)" className="w-full input-field" />
                        </div>
                        <div className="col-span-2">
                ...
                <div className="p-5 border-t border-gray-100 dark:border-gray-800 flex gap-3">
                    ...
                    <button className="btn-primary flex-1 justify-center" onClick={onClose}>Add Product</button>
                </div>
            </div>
        </div>
    );
                                ...
                                activeNav === 'overview' ? 'Dashboard Overview' :
                                    activeNav === 'products' ? 'Product Management' :
                                        activeNav === 'orders' ? 'Order Management' :
                                            activeNav === 'users' ? 'User Management' : 'Inventory'
                            }</h1>
                ...
                <main className="flex-1 p-4 sm:p-6 space-y-6">
                    ...
                    {activeNav === 'overview' && (
                        <>
                            ...
                            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                                ...
                                <StatCard label="Total Products" value={products.length} change={5.1} icon={Package} color="bg-purple-600" />
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                                ...
                                <div className="bg-gray-900 rounded-2xl p-5 border border-gray-800">
                                    <div className="flex items-center gap-2 mb-4">
                                        ...
                                        <span className="badge bg-orange-500/20 text-orange-400 text-xs ml-auto">{adminLowStock.length} items</span>
                                    </div>
                                    <div className="space-y-3">
                                        {adminLowStock.map(p => (
                                            <div key={p.id} className="flex items-center gap-3 p-3 rounded-xl bg-orange-500/10 border border-orange-500/20">
                                    ...
                                    <tbody className="divide-y divide-gray-800">
                                        {filteredProducts.map(p => (
                                            <tr key={p.id} className="hover:bg-gray-800/30 transition-colors">
                                                ...
                                                <td className="px-4 py-3 text-gray-300 capitalize">{p.partType}</td>
                    ...
                    {activeNav === 'inventory' && (
                        <div className="space-y-6">
                            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                                <StatCard label="Total SKUs" value={products.length} change={0} icon={Package} color="bg-blue-600" />
                                <StatCard label="In Stock" value={products.filter(p => p.stock > 0).length} change={3.2} icon={TrendingUp} color="bg-green-600" />
                                <StatCard label="Low Stock" value={adminLowStock.length} change={-15} icon={AlertTriangle} color="bg-orange-500" />
                                <StatCard label="Out of Stock" value={products.filter(p => p.stock === 0).length} change={20} icon={Package} color="bg-red-600" />
                            </div>
                            ...
                            <div className="bg-gray-900 rounded-2xl border border-gray-800 overflow-hidden">
                                ...
                                <div className="overflow-x-auto">
                                    <table className="w-full text-sm">
                                        ...
                                        <tbody className="divide-y divide-gray-800">
                                            {[...adminLowStock, ...products.filter(p => p.stock === 0)].map(p => (
                                                <tr key={p.id} className="hover:bg-gray-800/30 transition-colors">
                                                    <td className="px-4 py-3">
                                                        <div className="flex items-center gap-3">
